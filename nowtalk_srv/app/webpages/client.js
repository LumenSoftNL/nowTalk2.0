...
$(document).ready(function () {
  ...
  socket.on("config", (obj) => {
    $("#sts_server_name").text(obj.server_name);
    $("#sts_version").text(
      obj.version + "/" + (obj.bridge.version || "<None>")
    );
    $("#sts_externIP").text(obj.external_ip || "<None>");
  ...
  socket.on("update", (list, time) => {
    list.forEach((item) => {
      ...
      let statusClass =
        "nowtalk-status text-" + status_color + " me-1 fas fa-" + status_icon;

      if (table.has("#" + item.mac).length == 0) {
        let newRow = "<tr id='" + item.mac + "' class='align-middle'>";
        newRow += "<td class='text-center'>";
        newRow +=
          '<i data-action="status"  class="' +
          statusClass +
          '" title="' +
          status_hint +
          '" data-bs-toggle="dropdown" aria-expanded="false" role="button" tabindex="0"></i>';
        newRow +=
          "<ul class='dropdown-menu dropdown-menu-dark ' style='min-width:none'><li>";

        newRow += '<div class="p-0 btn-group">';
        newRow +=
          '<button class="dropdown-item nowtalk-toggle" data-action="friend" title="Make a friend"><i class="fas fa-users" ></i></button> ';
        newRow +=
          '<button class="dropdown-item nowtalk-edit" data-action="name" title="Edit username"><i class="fas fa-edit" ></i></button> ';
        newRow +=
          '<button class="dropdown-item text-danger nowtalk-toggle" data-action="disable" title="Block badge"><i class="fas fa-times" ></i></button> ';
        newRow += "</div></li></ul";
      ...
      row
        .find('[data-action="friend"]')
        .prop("disabled", (item.status & 0x10) !== 0)
        .attr("data-status", (item.status & 0x20) !== 0);
      row
        .find("[data-action='name']")
        .prop("disabled", (item.status & 0x30) === 0)
        .attr("data-name", item.name);
      row
        .find('[data-action="disable"]')
        .attr("data-status", (item.status & 0x80) !== 0);
  ...
  function callAddMessage(kind, msg) {
    ...
    switch (kind) {
      case "recv":
        _msg.addClass(
          "col-10 px-1 mt-1 text-success border rounded-end alert-secondary  "
        );
        break;
      case "send":
        row.append("<div class='col-2'></div>");
        _msg.addClass(
          "text-end col-10 mt-1 px-1 text-primary rounded-start alert-secondary "
        );
        break;
  ...
  $("#badgeid").on("input", function (event) {
    ...
    if (!checkBadgeID(id)) {
      email.setCustomValidity(
        "This is an invalid badgeID. The code is case sensitive!"
      );
    } else {
      email.setCustomValidity("");
    }
