...
class badge extends EventEmitter {
  ...
  fillInfo(userInfo) {
    ...
    this._mac =
      "h" + ("000000000000000" + userInfo.mac.toString(16)).substr(-12);
  ...
  info(useHexMac = false) {
    ...
    if (useHexMac) {
      if (ip == this.main.config.external_ip && this.isStatus(0x10)) ip = "";
      if (this.isStatus(0x08)) ip = this.external_ip;
      time = parseInt(
        Number(hrtime.bigint() - this.timestamp) /
          (this.main.config.badge_timeout * 10000000)
      );
    ...
    return {
      mac: useHexMac ? this._mac : this.mac,
      status: this.status,
      ip: ip,
      name: this.name,
      key: this.badgeID,
...
*/
  ...
  onResendRequest(nsg) {
    if (this.lastMessage != null && this.lastMessage.count < 5) {
      ...
      this.webMessage(
        "danger",
        `Badge ${this.badgeID} does not response anymore`
      );
    }
    this.lastMessage = null;
    return false;
  ...
  onPingPong(msg) {
    ...
    if (this.isStatus(0x80)) {
      ...
      this.webMessage(
        "error",
        `Badge ${this.badgeID} is blocked and ignored.`,
        this
      );
      this.main.unPeer(this.mac);
      return true;
      ...
      this.webMessage(
        "warning",
        "Unknown badge: " + msg._mac + ",  requesting  info."
      );
      ...
      if (!this.checkUpdates()) {
        if (this.isStatus(0x01)) {
          ...
          this.sendMessage(
            0x02,
            this.main.config.server_name + "~" + new Date().toJSON()
          );
        }
        this.off("handle_h30", this.onCallRequest);
